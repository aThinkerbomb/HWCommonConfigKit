...
#define IS_CHINESE [WXTContext sharedInstance].isChinese
...
#define isiPhoneXSize NO
#define TABBAR_HEIGHT 49
#define BATTERY_HEIGHT (isiPhoneXSize ? 44 : 20)
#define NAVBAR_HEIGHT  (44 + BATTERY_HEIGHT)   //每日头条红色背景高度 含状态条
...
#pragma mark - 事件
//#define AS_EVENT_DISPACTER @property (nonatomic, strong) void(^onActionInvoke)(WXTEventType event,id sender,NSDictionary* uInfo);\
@property (nonatomic, weak) id<WXTCommonEventDelegate> eventDelegate;\
-(void)notifiEvent:(WXTEventType)event withSender:(nullable id)sender withUserInfo:(NSDictionary *)uInfo;
//#define DEF_EVENT_DISPACTER -(void)notifiEvent:(WXTEventType)event withSender:(id )sender withUserInfo:(NSDictionary *)uInfo{\
